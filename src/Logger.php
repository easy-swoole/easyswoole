...
class Logger extends \EasySwoole\Trace\Logger
{
    ...
    function log(string $str, $category = 'default')
    {
        ...
        if(Config::getInstance()->getConf('CONSOLE.SOCKET_LOG')){
            TcpService::push("socketLog ".date("y-m-d H:i:s").":{$str}");
        }
    }
    ...
    function console(string $str, $saveLog = 1)
    {
        ...
        if(!$saveLog && Config::getInstance()->getConf('CONSOLE.SOCKET_LOG')){
            TcpService::push("socketLog ".date("y-m-d H:i:s").":{$str}");
        }
    }
}