...
class Core
{
    ...
    public function start()
    {
        ...
        //执行Actor注册进程
        Actor::getInstance()->setTempDir(EASYSWOOLE_TEMP_DIR)->setServerName($serverName)->attachToServer(
    ...
    private function registerDefaultCallBack(\swoole_server $server, int $serverType)
    {
        ...
        if ($serverType === EASYSWOOLE_SERVER) {
            ...
            } elseif (in_array($socketType, [SWOOLE_UDP, SWOOLE_UDP6])) {
            ...
            EventHelper::on(
                ...
                function (\swoole_http_request $request, \swoole_http_response $response) use ($dispatcher) {
                    ...
                    try {
                        if (EasySwooleEvent::onRequest($requestPsr, $responsePsr)) {
                            $dispatcher->dispatch($requestPsr, $responsePsr);
                        }
                    } catch (\Throwable $throwable) {
                        call_user_func(
                            Di::getInstance()->get(SysConst::HTTP_EXCEPTION_HANDLER),
                            $throwable,
                            $requestPsr,
                            $responsePsr
                        );
                        ...
                        try {
                            EasySwooleEvent::afterRequest($requestPsr, $responsePsr);
                        } catch (\Throwable $throwable) {
                            call_user_func(
                                Di::getInstance()->get(SysConst::HTTP_EXCEPTION_HANDLER),
                                $throwable,
            ...
            EventHelper::on($server, EventRegister::onTask, function (\swoole_server $server, Task $task) {
                ...
                if (is_string($taskObj) && class_exists($taskObj)) {
                    ...
                    if ($ref->implementsInterface(QuickTaskInterface::class)) {
                        ...
                        return;
                    } elseif ($ref->isSubclassOf(AbstractAsyncTask::class)) {
                        $taskObj = new $taskObj;
                    }
                }
        ...
        EventHelper::registerWithAdd(
            ServerManager::getInstance()->getMainEventRegister(),
